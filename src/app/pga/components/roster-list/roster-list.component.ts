...
export class RosterListComponent implements OnInit, OnChanges {
  @Input() roster: PGARosterDto[] = [];
  @Input() isLoading: boolean = false;
  @Output() addRow = new EventEmitter<void>();
  @Output() editRow = new EventEmitter<PGARosterDto>();
  @Output() deleteRow = new EventEmitter<string | number>();

  filteredRoster: PGARosterDto[] = [];
  ...
  private buildTeamOptions(rows: PGARosterDto[]): string[] {
    const values = (rows || []).map((row) => this.getTeamValue(row)).filter((team) => !!team);
    return Array.from(new Set(values)).sort((a, b) => a.localeCompare(b));
  }

  private getTeamValue(row: PGARosterDto): string {
